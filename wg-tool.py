...
def create_public_key():
    ...
    public_key = (
        subprocess.check_output(("wg", "pubkey"), stdin=ps.stdout).decode().strip()
    )
    ps.wait()
...
def init_server_config(addr=None, do_forward=True):
    ...
    with open(config_file, "w") as fh:
        ...
        fh.write(
            "Address = {first_addr}/{pref_len}\n".format(
                first_addr=addr, pref_len=pref_len
            )
        )
...
def add_peer(client_public_key, client_addr):
    subprocess.check_output(
        ["wg", "set", WG_DEV, "peer", client_public_key, "allowed-ips", client_addr]
    )
...
def init_client_config(name, client_addr):
    ...
    config_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), name + ".conf"
    )
    ...
    with open(config_file, "w") as fh:
        ...
        fh.write(
            "Endpoint = {endpoint_ip}:{endpoint_port}\n".format(
                endpoint_ip=ENDPOINT, endpoint_port=LISTEN_PORT
            )
        )
...
def main():
    ...
    config_server.add_argument(
        "--addr", type=str, required=False, default=None
    )  # if not set will take first net addr
